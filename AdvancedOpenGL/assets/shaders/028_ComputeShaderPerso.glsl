...
uniform float rule1_weight = 0.10; //0.18
uniform float rule2_weight = 0.05; //0.05
uniform float rule3_weight = 0.05; //0.17
uniform float rule4_weight = 0.10; //0.02
...
vec3 rule4(vec3 my_position, vec3 my_velocity, vec3 their_position, vec3 their_velocity)
{
    vec3 dir_to_goal = normalize(goal - my_position);
    vec3 dir_circumferential = vec3(dir_to_goal.y, -dir_to_goal.x, 0.0);

    return dir_circumferential;
}
    ...
    for (i = 0; i < gl_NumWorkGroups.x; i++)
    {
        ...
        for (j = 0; j < gl_WorkGroupSize.x; j++)
        {
            ...
            if (i * gl_WorkGroupSize.x + j != global_id)
            {
                ...
                accelleration += rule4(me.position, me.velocity, them.position, them.velocity) * rule4_weight;
            }
        }
        barrier();
    }
