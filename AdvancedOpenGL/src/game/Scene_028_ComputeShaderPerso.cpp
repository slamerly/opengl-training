#include "Scene_028_ComputeShaderPerso.h"
...
Scene_028_ComputeShaderPerso::Scene_028_ComputeShaderPerso() : totalTime(0), frameIndex(0) {

}

Scene_028_ComputeShaderPerso::~Scene_028_ComputeShaderPerso() {
    clean();
}

void Scene_028_ComputeShaderPerso::setGame(Game *_game) {
    game = _game;
}

void Scene_028_ComputeShaderPerso::clean() {

}

void Scene_028_ComputeShaderPerso::pause() {
}

void Scene_028_ComputeShaderPerso::resume() {
}

void Scene_028_ComputeShaderPerso::handleEvent(const InputState &inputState) {

}

void Scene_028_ComputeShaderPerso::load() {
...
void Scene_028_ComputeShaderPerso::update(float dt) {
    totalTime += dt;

    computeShader.use();
    //Vector3 goal = Vector3(sinf(totalTime * 0.34f), cosf(totalTime * 0.29f), sinf(totalTime * 0.12f) * cosf(totalTime * 0.5f));

    Vector3 goal = Vector3(cosf(totalTime * 0.25f), sinf(totalTime * 0.12f), sinf(totalTime * 0.05f));


    //i * std::cos(3 * t), i * std::sin(3 * t), 0.0f)
...
void Scene_028_ComputeShaderPerso::draw()
{
    renderShader.use();
    ...
    Matrix4 proj_matrix = Matrix4::createPerspectiveFOV(60.0f, game->windowWidth, game->windowHeight, 0.1f, 1000.0f);
    Matrix4 mvp = proj_matrix * mv_matrix;

    renderShader.setMatrix4("mvp", mvp);

    glBindVertexArray(flockRenderVao[frameIndex]);
    glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, 20, FLOCK_SIZE);
    //glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, 36, FLOCK_SIZE);
